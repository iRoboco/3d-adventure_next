...
/// @}
// ============================================================================
/** @name Fur-меш (Shell-метод)
 *  Генерация геометрии для меха травы на верхних гранях блоков BLOCK_GRASS
 */
/// @{

/**
 * @brief Генерирует меш из квадов на верхних гранях травяных блоков для shell-рендеринга.
 *
 * Каждый квад соответствует верхней грани блока BLOCK_GRASS, над которым — воздух.
 * Вершины имеют нормаль (0,1,0) для смещения в shell-шейдере.
 * UV-координаты берутся из тайла травы (индекс 0) атласа 4x4.
 *
 * @param data Данные чанка с блоками
 * @param vertices Выходной вектор позиций (x,y,z триплеты)
 * @param normals Выходной вектор нормалей (nx,ny,nz триплеты)
 * @param uvs Выходной вектор UV-координат (u,v пары)
 * @param indices Выходной индексный буфер
 */
inline void buildFurMesh(const ChunkData& data,
                          ...
                          ax::IndexArray& indices)
{
    ...
    auto [u0, v0, u1, v1] = calculateTileUV(0);  // grass top tile
    ...
    for (int z = 0; z < CZ; ++z)
        for (int x = 0; x < CX; ++x)
            for (int y = 0; y <= yMax; ++y)
            {
                if (data.getBlock(x, y, z) != BLOCK_GRASS)
                    continue;
                if (y + 1 < CY && data.getBlock(x, y + 1, z) != BLOCK_AIR)
                    continue;
                ...
                uint16_t base = static_cast<uint16_t>(positions.size() / 3);

                // v0--v1
                // |    |
                // v3--v2  (CCW from top: v0→v1→v2→v3)
                float verts[] = {
                    x0, y1, z1,   u0, v1,   // v0
                    x1, y1, z1,   u1, v1,   // v1
                    x1, y1, z0,   u1, v0,   // v2
                ...
                for (int i = 0; i < 4; ++i)
                {
                    positions.push_back(verts[i * 5 + 0]);
                    positions.push_back(verts[i * 5 + 1]);
                    positions.push_back(verts[i * 5 + 2]);
                    normals.push_back(0.0f);
                    normals.push_back(1.0f);
                    normals.push_back(0.0f);
                    uvs.push_back(verts[i * 5 + 3]);
                    uvs.push_back(verts[i * 5 + 4]);
                }
...
inline ax::Mesh* createFurMesh(const std::vector<float>& positions,
